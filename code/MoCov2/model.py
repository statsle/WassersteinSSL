...
class MoCoplus(nn.Module):
    ...
    def _momentum_update_key_encoder(self, cur_step):
        ...
        current_momentum =  self.args.final_momentum - (self.args.final_momentum - self.args.base_momentum) * (math.cos(math.pi * cur_step / max_steps) + 1) / 2

        for param_q, param_k in zip(self.encoder_q.parameters(), self.encoder_k.parameters()):
            param_k.data = param_k.data * current_momentum + param_q.data * (1. - current_momentum)
    ...
    def build_model(self, x1, x2, step, lambd): 
        ...
        print_pack = Pack(total_loss = total_loss, contrastive_loss=contrastive_loss, wasserstein_loss=wasserstein_loss)
    ...
    def train_model(self, args):
        ...
        results = {'total_loss':[], 'contrastive_loss': [], 'wasserstein_loss':[], 'test_acc@1': [], 'test_acc@5': []}
        best_acc = 0.0
        start = time.time()
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for step, ((x1, x2), label) in enumerate(self.train_dataloader, start=(epoch-1)*len(self.train_dataloader)):
                iter_count += 1
                batch_size = x1.size(0)
